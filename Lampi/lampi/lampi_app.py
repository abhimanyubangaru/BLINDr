...
class LampiApp(App):
    ...
    # blind_info_button_pressed = BooleanProperty(False)
    ...
    gpio_BLIND_INFO_pressed = BooleanProperty(False)
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        self.mqtt.message_callback_add(TOPIC_BLIND_SET_PERCENTAGE,
                                       self.receive_new_blind_percentage)
        self.mqtt.message_callback_add(TOPIC_BLIND_TIMER,
                                       self.receive_new_timer)
        ...
        self.mqtt.subscribe(TOPIC_BLIND_SET_PERCENTAGE, qos=2)
        self.mqtt.subscribe(TOPIC_BLIND_TIMER, qos=2)
    ...
    def receive_new_blind_percentage(self, client, userdata, message):
        new_state = json.loads(message.payload.decode('utf-8'))
        self.blind_percentage = new_state['percentage']
        # Ensure UI updates happen on the main thread
        Clock.schedule_once(
            lambda dt: self._update_blind_info_popup(self.info_status_popup))

    def receive_new_timer(self, client, userdata, message):
        new_state = json.loads(message.payload.decode('utf-8'))
        self.time_to_close = new_state['timer']
        Clock.schedule_once(
            lambda dt: self._update_blind_info_popup(self.info_status_popup))
        Clock.schedule_once(lambda dt: self.close_popup(), 5)

    def close_popup(self):
        if self.popup_is_open:
            self.info_status_popup.dismiss()
            self.popup_is_open = False

    def _update_blind_info_popup(self, instance):
        if instance is not None:
            msg = "Blind Percentage: {}%\n BLINDS SCHEDULED\n TO CLOSE AT:\n{}".format(
                self.blind_percentage, self.time_to_close)
            instance.content.text = msg
            if not self.popup_is_open:
                instance.open()
    ...
    def set_up_GPIO_and_device_status_popup(self):
        ...
        self.pi.set_mode(BLIND_INFO_PIN, pigpio.INPUT)
        self.pi.set_pull_up_down(BLIND_INFO_PIN, pigpio.PUD_UP)
        ...
        # info_status_popup
        self.info_status_popup = self._build_info_status_popup()
        self.info_status_popup.bind(on_open=self._update_blind_info_popup)
    ...
    def _build_info_status_popup(self):
        return Popup(title='Blinds Timer',
                     content=Label(text='Timer will update shortly.'),
                     size_hint=(1, 1), auto_dismiss=False)
    ...
    def on_gpio_BLIND_INFO_pressed(self, instance, value):
        if value:
            self.info_status_popup.open()
        else:
            self.info_status_popup.dismiss()
    ...
    def _poll_GPIO(self, dt):
        ...
        self.gpio_BLIND_INFO_pressed = not self.pi.read(BLIND_INFO_PIN)
